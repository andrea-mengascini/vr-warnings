generate_sample <- function(def_population, expected_effect, sample_size = meta$sample_size) {
  ...
  mydt <- genData(
    ...
    #
    # We have 16 trials per participant.
    genCluster(
  ...
  ret[, warning := ifelse(warning == 0, 0, warning_type)]
  ret[, warning := factor(warning, levels = 0:4, labels = c("nowarning", "blur", "red", "scaledown", "popup"))]
...
fit_model <- function(mysample) {
  lmerTest::lmer(
    # Question and object are too unstable in our simulation. No sense to
    # include them.
    #answer_r ~ 1 + warning + (1 | id_participant) + (1 | question_id) + (1 | object_id),
    answer_r ~ 1 + warning + (1 | id_participant),
    data = mysample
  )
}
...
check_model_result <- function(mymodel) {
  ...
  if (length(mymodel@optinfo$warnings) != 0 || lme4::isSingular(mymodel)) {
    ...
    # Call the summary to get the p-value of the lmerTest object. This is
